...
class TestBasic:
    def test_make_dotdict(self):
        ...
        assert dd.make_dotdict(None) is None
        assert dd.make_dotdict(3) == 3
        assert dd.make_dotdict('3') == '3'
        assert dd.make_dotdict(['3', 3]) == ['3', 3]
        assert dd.make_dotdict(('3', 3)) == ['3', 3]

        # Valid dict. Verify Dot access.
        d = dd.make_dotdict({'foo': 'bar'})
        assert d.foo == 'bar'

        # A list of dicts.
        d = dd.make_dotdict([{'foo0': 'bar0'}, {'foo1': 'bar1'}])
        assert d[0].foo0 == 'bar0'
        assert d[1].foo1 == 'bar1'

        # A dict of dicts.
        d = dd.make_dotdict({'foo0': {'foo1': 'bar1'}})
    ...
    def test_undo_dotdict(self):
        ...
        ddict_undo = dd.undo_dotdict(ddict)
        assert type(ddict_undo) == dict
